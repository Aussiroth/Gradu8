...
public class BlacklyManager : MonoBehaviour {

    private List<Blackly> jellies = new List<Blackly>();
    ...
    private Blackly GetJelly()
    {
        Blackly j = jellies.Find(x => !x.IsActive);

        if (j == null)
        {
            j = Instantiate(jellyPrefab).GetComponent<Blackly>();
            jellies.Add(j);
        }
